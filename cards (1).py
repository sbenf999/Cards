...
class Card:
    ...
    def generate_card(self):
        ...
        '''
        self.suit = random.choice(["Diamonds", "Hearts", "Clubs", "Spades"])
...
class Deck(Card):
    ...
    def new_deck(self, jokerHL, joker=False, aceLow=True, card_defaults=[2, 3, 4, 5, 6, 7, 8, 9, 10, 10, 10, 10, 1]):
        ...
        for i in range(4):
            for j in range(len(card_defaults)):
                ...
                new_card = Card(["Diamond", "Hearts", "Clubs", "Spades"][i], self.ranks[j], card_defaults[j])
    ...
    def shuffle(self):
        ...
        for v, letter in enumerate(array):
            random_num = random.randint(0, (len_of_array-1))
            
            while random_num in already_used:
                random_num = random.randint(0, (len_of_array-1))
            
            already_used.append(random_num)
            array[v], array[random_num] = array[random_num], array[v]
...
new_deck = Deck()
new_deck.new_deck(True, joker=False)
new_deck.shuffle()
print(new_deck)
print(new_deck.size())
